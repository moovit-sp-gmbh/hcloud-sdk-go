...
func (c *Client) GetEvent(organization string, app string, event string) (*entities.Event, *http.Response, *hcloud.ErrorResponse) {
...
func (c *Client) DeleteEvent(organization string, app string, event string) (*http.Response, *hcloud.ErrorResponse) {
	resp, _, err := c.client.Delete(c.getEndpoint()+fmt.Sprintf("/v1/org/%s/apps/%s/events/%s", organization, app, event), nil)
	if err != nil {
		return nil, err
	}
