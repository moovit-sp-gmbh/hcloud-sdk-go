...
func (c *Client) DeleteWebhookById(organization string, app string, webhookId string) (*http.Response, *hcloud.ErrorResponse) {
	resp, _, err := c.client.Delete(c.getEndpoint() + fmt.Sprintf("/v1/org/%s/apps/%s/webhooks/%s", organization, app, webhookId))
	if err != nil {
		return nil, err
	}
